...
class QuePaxaClientWorker : public ClientWorker {
 ...
 public:
  ...
  void Work() override {
    #ifdef QUEPAXA_TEST_CORO
    testconfig_ = new QuePaxaTestConfig(QuePaxaFrame::replicas_);  // todo: destroy
    Coroutine::CreateRun([this] () {
      QuePaxaLabTest test(testconfig_);
      test.Run();
      Coroutine::Sleep(10);
    ...
    for (uint32_t n_tx = 0; n_tx < n_concurrent_; n_tx++) {
      auto sp_job = std::make_shared<OneTimeJob>([this, n_tx, &cmd_leader, &cmds] () {
        ...
        while (n_tx_issued_ < tot_req_num_) {
          ...
          n_tx_issued_++;
          QuePaxaCommo* quepaxa_commo = dynamic_cast<QuePaxaCommo*>(commo_);
          quepaxa_commo->SendStart(svr, 0, cmd, [this]() {
            n_done_tx_++;
          });
        }
    ...
    auto sp_metrics_job = std::make_shared<OneTimeJob>([this, &fast, &commit_times, &exec_times] () mutable {
      ...
      for (int svr = 0; svr < NSERVERS; svr++) {
        ...
        auto ev = quepaxa_commo->CollectMetrics(svr, 0, &fast_, &commit_times_, &exec_times_);
        ev->Wait(1000000);
        fast += fast_;
